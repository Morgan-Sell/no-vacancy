...
def test_import_csv_and_log(temp_booking_data_csv, test_db_conn):
    ...
    with open(temp_booking_data_csv, "r") as f:
        ...
        with open(filtered_csv_path, "w") as filtered_csv:
            ...
            for line in f:
                ...
                indices = [header.index("number of adults"), header.index("number of weekend nights")]
    ...
    assert not has_been_imported(test_db_conn, filtered_csv_path, file_hash), "CSV should be not marked as imported"
    ...
    assert has_been_imported(test_db_conn, filtered_csv_path, file_hash), "CSV should be marked as imported"
